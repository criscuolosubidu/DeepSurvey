...
class subsectionWriter():
    ...
    def __init__(self, model:str, api_key:str, api_url:str, database, language="zh") -> None:
        ...
        # 导入所需的提示词常量
        self.SUBSECTION_WRITING_PROMPT = getattr(prompts_module, "SUBSECTION_WRITING_PROMPT")
        self.LCE_PROMPT = getattr(prompts_module, "LCE_PROMPT")
        self.CHECK_CITATION_PROMPT = getattr(prompts_module, "CHECK_CITATION_PROMPT")
    ...
    def write_subsection_with_reflection(self, paper_texts_l, topic, outline, section, subsections, subdescriptions, res_l, idx, rag_num = 20, subsection_len = 1000, citation_num = 8):
        ...
        for j in range(len(subsections)):
            ...
            prompt = self.__generate_prompt(self.SUBSECTION_WRITING_PROMPT, paras={'OVERALL OUTLINE': outline,'SUBSECTION NAME': subsection,\
        ...
        for content, paper_texts in zip(contents, paper_texts_l):
            prompts.append(self.__generate_prompt(self.CHECK_CITATION_PROMPT, paras={'SUBSECTION': content, 'TOPIC':topic, 'PAPER LIST':paper_texts}))
        self.input_token_usage += self.token_counter.num_tokens_from_list_string(prompts)
        contents = self.api_model.batch_chat(prompts, temperature=1)
        self.output_token_usage += self.token_counter.num_tokens_from_list_string(contents)
        contents = [c.replace('<format>','').replace('</format>','') for c in contents]
    ...
    def lce(self, topic, outline, contents, res_l, idx):
        ...
        prompt = self.__generate_prompt(self.LCE_PROMPT, paras={'OVERALL OUTLINE': outline,'PREVIOUS': contents[0],\
    ...
    def parse_outline(self, outline):
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('# '):
                ...
                if i + 1 < len(lines) and (lines[i + 1].startswith('Description:') or lines[i + 1].startswith('描述：')):
                    if lines[i + 1].startswith('Description:'):
                        result["section_descriptions"].append(lines[i + 1].split('Description:', 1)[1].strip())
                    else:
                        result["section_descriptions"].append(lines[i + 1].split('描述：', 1)[1].strip())
                ...
                if result["subsections"]:
                    ...
                    if i + 1 < len(lines) and (lines[i + 1].startswith('Description:') or lines[i + 1].startswith('描述：')):
                        if lines[i + 1].startswith('Description:'):
                            ...
                            result["subsection_descriptions"][-1].append(lines[i + 1].split('描述：', 1)[1].strip())
    ...
    def parse_survey(self, outline):
        ...
        for i in range(100):
            ...
            elif f'子部分{i+1}' in outline or f'子部分 {i+1}' in outline:
                # 处理中文格式，兼容有空格和没空格的情况
