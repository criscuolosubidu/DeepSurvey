...
class outlineWriter():
    ...
    def __init__(self, model:str, api_key:str, api_url:str, database, language="zh") -> None:
        ...
        # 根据language参数选择不同的提示词模块
        if language.lower() in ["zh", "chinese", "中文"]:
            prompts_module = importlib.import_module("src.prompts_zh")
        else:
            prompts_module = importlib.import_module("src.prompts")
        
        # 导入所需的提示词常量
        self.ROUGH_OUTLINE_PROMPT = getattr(prompts_module, "ROUGH_OUTLINE_PROMPT")
        self.MERGING_OUTLINE_PROMPT = getattr(prompts_module, "MERGING_OUTLINE_PROMPT")
        self.SUBSECTION_OUTLINE_PROMPT = getattr(prompts_module, "SUBSECTION_OUTLINE_PROMPT")
        self.EDIT_FINAL_OUTLINE_PROMPT = getattr(prompts_module, "EDIT_FINAL_OUTLINE_PROMPT")
    ...
    def generate_rough_outlines(self, topic, papers_chunks, titles_chunks, section_num = 8):
        ...
        for i in trange(len(papers_chunks)):
            ...
            prompt = self.__generate_prompt(self.ROUGH_OUTLINE_PROMPT, paras={'PAPER LIST': paper_texts, 'TOPIC': topic, 'SECTION NUM': str(section_num)})
    ...
    def merge_outlines(self, topic, outlines):
        ...
        for i, o in zip(range(len(outlines)), outlines):
            outline_texts += f'---\noutline_id: {i}\n\noutline_content:\n\n{o}\n'
        outline_texts+='---\n'
        prompt = self.__generate_prompt(self.MERGING_OUTLINE_PROMPT, paras={'OUTLINE LIST': outline_texts, 'TOPIC':topic})
    ...
    def generate_subsection_outlines(self, topic, section_outline, rag_num):
        ...
        for section_name, section_description in zip(survey_sections, survey_section_descriptions):
            ...
            for i, t, p in zip(range(len(references_papers)), references_titles, references_papers):
                paper_texts += f'---\npaper_title: {t}\n\npaper_content:\n\n{p}\n'
            paper_texts+='---\n'
            prompt = self.__generate_prompt(self.SUBSECTION_OUTLINE_PROMPT, paras={'OVERALL OUTLINE': section_outline,'SECTION NAME': section_name,\
    ...
    def edit_final_outline(self, outline):
        ...
        prompt = self.__generate_prompt(self.EDIT_FINAL_OUTLINE_PROMPT, paras={'OVERALL OUTLINE': outline})
        self.input_token_usage += self.token_counter.num_tokens_from_string(prompt)
        outline = self.api_model.chat(prompt, temperature=1)
        self.output_token_usage += self.token_counter.num_tokens_from_string(outline)
    ...
    def extract_title_sections_descriptions(self, outline):
        if outline is None:
            print("警告: outline 是 None，无法提取标题和章节")
            return "未知标题", [], []
        ...
        if not isinstance(outline, str):
            try:
                ...
                print(f"警告: outline 不是字符串且无法转换为字符串: {type(outline)}")
                return "未知标题", [], []
        ...
        if 'Title: ' in outline:
            ...
            title = outline.split('标题：')[1].split('\n')[0]
        ...
        for i in range(100):
            if f'Section {i+1}: ' in outline:
                ...
                if f'Description {i+1}: ' in outline:
                    descriptions.append(outline.split(f'Description {i+1}: ')[1].split('\n')[0])
        
        # 如果没有找到英文格式，检查中文格式
        if len(sections) == 0:
            for i in range(100):
                if f'第{i+1}部分：' in outline:
                    sections.append(outline.split(f'第{i+1}部分：')[1].split('\n')[0])
                    if f'描述{i+1}：' in outline:
                        descriptions.append(outline.split(f'描述{i+1}：')[1].split('\n')[0])
                # 处理有空格的情况
                elif f'第 {i+1} 部分：' in outline:
                    sections.append(outline.split(f'第 {i+1} 部分：')[1].split('\n')[0])
                    if f'描述 {i+1}：' in outline:
                        descriptions.append(outline.split(f'描述 {i+1}：')[1].split('\n')[0])
    ...
    def extract_subsections_subdescriptions(self, outline):
        if outline is None:
            print("警告: outline 是 None，无法提取子章节")
            return [], []
        ...
        for i in range(100):
            if f'Subsection {i+1}: ' in outline:
                try:
                    ...
                    print(f"警告: 解析 Subsection {i+1} 或其描述时出现索引错误")
        
        # 如果没有找到英文格式，检查中文格式
        if len(subsections) == 0:
            for i in range(100):
                # 处理无空格的情况
                if f'子部分{i+1}：' in outline:
                    try:
                        subsections.append(outline.split(f'子部分{i+1}：')[1].split('\n')[0])
                        if f'描述{i+1}：' in outline:
                            subdescriptions.append(outline.split(f'描述{i+1}：')[1].split('\n')[0])
                    except IndexError:
                        print(f"警告: 解析子部分{i+1}或其描述时出现索引错误")
                
                # 处理有空格的情况
                elif f'子部分 {i+1}：' in outline:
                    try:
                        subsections.append(outline.split(f'子部分 {i+1}：')[1].split('\n')[0])
                        if f'描述 {i+1}：' in outline:
                            subdescriptions.append(outline.split(f'描述 {i+1}：')[1].split('\n')[0])
    ...
    def process_outlines(self, section_outline, sub_outlines):
        if section_outline is None:
            print("警告: section_outline 是 None，无法处理大纲")
            return "# 未能生成大纲\n\n请重试或检查系统设置。"
        
        if not isinstance(sub_outlines, list) or len(sub_outlines) == 0:
            print(f"警告: sub_outlines 不是有效的列表或为空: {type(sub_outlines)}")
            return "# 未能生成完整大纲\n\n无法处理子章节。"
        ...
        try:
            ...
            for i in range(len(survey_sections)):
                ...
                res += f'## {i+1} {section}\n描述：{survey_section_descriptions[i]}\n\n'
                
                # 确保 i 在 sub_outlines 的索引范围内
